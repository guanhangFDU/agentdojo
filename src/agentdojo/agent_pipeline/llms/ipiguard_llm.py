...
def _openai_to_assistant_message(message: ChatCompletionMessage) -> ChatAssistantMessage:
    ...
    if message.content is None:
        ...
        content_blocks = [text_content_block_from_string(message.content)]
  ...
  Follow these instructions:
    ...
    def _get_system_message(
        ...
        return ChatSystemMessage(
            role="system",
            content=[text_content_block_from_string(message_content)],
        )
    ...
    def query(
        ...
        completion = get_pre_plan(
            client=self.client,
            model=self.model,
            temperature=self.temperature,
            user_instruction=query,
            tool_docs=get_tool_docs(list(runtime.functions.values())),
        )
        pre_plan = completion.choices[0].message.content
        prompt_tokens, completion_tokens = completion.usage.prompt_tokens, completion.usage.completion_tokens
        add_tokens(extra_args=extra_args, prompt_tokens=prompt_tokens, completion_tokens=completion_tokens)
...
class OpenAITraverseLLM(OpenAILLM):
    ...
    def _prepare_system_prompt(self) -> ChatSystemMessage:
        return ChatSystemMessage(
            role="system",
            content=[text_content_block_from_string(self._args_update_prompt)],
        )
    ...
    def _prepare_user_prompt(self, 
        ...
        return ChatUserMessage(
            role="user",
            content=[text_content_block_from_string(user_message)],
        )
